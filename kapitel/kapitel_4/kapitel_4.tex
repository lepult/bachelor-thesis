...
Die Positionen sowie weitere Statusinformationen der Roboter, wie beispielsweise der aktuelle Auftrag und Akkuladung, werden mithilfe einer indirekten \gls{Websocket}-Verbindung zwischen der Webanwendung und dem \ac{BCB} regelmäßig aktualisiert. Hierfür wird der für chayns interne Websocket-Service genutzt. In der folgenden Abbildung ist die Struktur des Websocket-Services abgebildet. AKtualisiert ein Roboter seine Position, wird die entsprechende Information über \gls{MQTT} an das \ac{BCB} gesendet.
% Passt das so? Funktioniert MQTT wie beschrieben?
Das \ac{BCB} sendet daraufhin eine Nachricht an den Websocket-Service, der diese Information wiederum an alle Clients schickt, die mit diesem verbunden sind.
% TODO Schaubild einfügen.

Die über den Websocket-Service empfangenen aktualisieren Informationen werden zentral im Redux-Store aktualisiert, sodass die aktualisierten Informationen dem Nutzer direkt angezeigt werden können. Damit aktualisierte Informationen in den \deckgl{}-Layers angezeigt werden, wird in diesen die updateTriggers Property gesetzt.
...
% getTooltip Quelle
Es gibt zudem die Möglichkeit einem Roboter zu Folgen. Die Kamera wird hierfür mithilfe des FlyToInterpolators zu dem Roboter bewegt. 
